			...
			<section id="about" class="about quote-region double-diagonal pt-5" style="<?php if (count($errors) > 0) { echo 'display:none;'; } ?>">
				...
				<div class="container double-diagonal">
					...
					<div class="row ">
									...
									<div class="col-sm-12 col-md-6 col-lg-6 pb-0 pb-lg-3">
										...
										<div class="col-sm-12 col-md-6 col-lg-6">
											...
											<div>
												...
												<div> <label for="coverageAmount">Amount of Coverage ($)
													<span class="field-tip">
														<img class="" style="width:15px;height:14px;margin-left:2px;margin-right:2px; padding-bottom:0px;" src="/img/info.png" id="img_insured_ssn">
														<span class="tip-content">Go <a style="color:#ff0384;" href="/free-life-insurance-calculator">here</a> if you're not sure how much you need </span></span>:</label></div>

														<input type="number" min="100000" max="1000000000" class="w-100" name="coverageAmount" id="coverageAmount" value="<?php if(isset($_GET['coverageAmount']) && $_GET['coverageAmount'] != '') {echo strip_tags($_GET['coverageAmount']); }  ?>"
						...
						<script>
				//If you came from the calculator page, let's clarify the coverage amount number
				
				if (window.location.href.indexOf("?coverageAmount=") >= 0) {
								$('label[for="coverageAmount"]').text('Amount of Coverage Based on Your Needs:');
							} 
	...
	jQuery('#quote-form-2').submit(function(){
      ...
      jQuery('#zipcode').inputmask({"mask": "99999[-9999]"},{greedy:false}); //specifying options
        ...
        jQuery("#ssn").inputmask("999-99-9999",{placeholder:" ", clearMaskOnLostFocus: true }); //default
        	...
        	//This is a cool idea but doesn't display an error when you put in amounts over the min-max range since you have to set input type to text instead of number
        // jQuery('#coverageAmount').inputmask({ 
        // 	alias: 'currency', 
        // 	digits: 2,
        // 	rightAlign: 0,
